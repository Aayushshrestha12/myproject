...
try {
    if($vendor_service_id){
        // Update existing
        $stmt = $conn->prepare("UPDATE vendor_service SET service_id=?, price=?, available_from=?, available_to=? WHERE id=? AND vendor_id=?");
        $stmt->bind_param("iddsii", $service_id, $price, $available_from, $available_to, $vendor_service_id, $vendor_id);
    } else {
        // Insert new
        $stmt = $conn->prepare("INSERT INTO vendor_service (vendor_id, service_id, price, available_from, available_to) VALUES (?, ?, ?, ?, ?)");
