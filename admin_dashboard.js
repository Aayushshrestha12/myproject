...
// for approving/rejecting vendors
        ...
        btn.addEventListener('click', function() {
            ...
            .then(data => {
                if(data.success) {
                    ...
                    if(row) row.remove(); // remove from table
                    showToast(`Vendor ${action}d successfully!`);
                } else {
                    alert(data.message);
                }
            })
...
document.querySelectorAll('#manage-provider a').forEach(link => {
...
function updateUser(userId) {
    ...
    xhr.open("POST", "user_action_ajax.php", true);
...
function deleteUser(userId) {
    ...
    xhr.send(JSON.stringify({id: userId, action: "delete"}));
}
