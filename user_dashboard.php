...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['search_service'])) {
    ...
    $sql = "SELECT s.service_id, s.category, s.description,
                   v.vendor_id, v.name AS vendor_name, v.location, v.skills,v.phone,
    ...
    <div class="dashboard-container">
        ...
        <div class="sidebar">
            ...
            <nav class="nav-menu">
                ...
                <button class="nav-item" data-page="bookings">📖 My Bookings</button>
        ...
        alt="Profile">
                ...
                <div class="search-results">
                    ...
                    <div class="vendor-card">
                        ...
                        <p class="vendor-price">₹<?= $row['price'] ?? 0 ?>/hour</p>
                        ...
                        <div class="booking-card">
                            ...
                            <div class="booking-header">
                                ...
                                <span class="booking-status <?= strtolower($b['status']) ?>">
                                    <?= ucfirst(strtolower(htmlspecialchars($b['status']))) ?>
                                </span>
                            </div>
                            ...
                            <div class="booking-info">
                                ...
                                <p>
                                    Vendor: <?= htmlspecialchars($b['vendor_name']) ?>
                                    (<?= htmlspecialchars($b['location']) ?>)<br>
                                    Date: <?= htmlspecialchars($b['booking_date']) ?> | Time:
                                    <?= htmlspecialchars($b['booking_time']) ?><br>
                                    <strong>Price: ₹<?= number_format($b['price'],2) ?></strong>
                                </p>
                            </div>
                            ...
                            <div class="booking-actions">
                                ...
                                <!-- Payment Button -->
                                <?php if (strtolower($b['status']) === 'completed' && strtolower($b['payment_status']) === 'unpaid'): ?>
                                <form action="esewa_payment.php" method="POST" class="payment-form">
                                    <input type="hidden" name="booking_id"
                                        value="<?= htmlspecialchars($b['booking_id']) ?>">
                                    <input type="hidden" name="amount" value="<?= htmlspecialchars($b['price']) ?>">
                                    <input type="hidden" name="vendor_id"
                                        value="<?= htmlspecialchars($b['vendor_id']) ?>">
                                    <input type="hidden" name="service_id"
                                        value="<?= htmlspecialchars($b['service_id']) ?>">
                                    <input type="hidden" name="pay_method" value="esewa">
                                    <button type="submit" class="pay-btn">💳 Pay via eSewa</button>
                                </form>
                                <?php elseif (strtolower($b['payment_status']) === 'paid'): ?>
                                <span class="paid-label">✅ Payment Completed</span>
                                <?php endif; ?>
