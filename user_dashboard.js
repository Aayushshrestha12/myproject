...
document.getElementById('editReviewForm').addEventListener('submit', function(e){
    e.preventDefault(); // ✅ Must be inside function
    ...
    .then(data => {
        if(data.success){
            // ✅ Make sure your review card has an ID like review-<id>
            const card = document.getElementById('review-' + reviewId);
            if(card){
                card.querySelector('.review-text').innerHTML = text.replace(/\n/g,'<br>');
                const stars = "★".repeat(rating) + "☆".repeat(5 - rating);
                card.querySelector('.rating').innerHTML = `${stars} <span>${parseFloat(rating).toFixed(1)}</span>`;
            }
