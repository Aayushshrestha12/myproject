...
if($action === 'save'){
    ...
    if($service_id){ // Update
        ...
        $linkStmt = $conn->prepare("INSERT INTO vendor_service (vendor_id, service_id) VALUES (?, ?)");
...
if($action === 'delete' && isset($_POST['service_id'])){
    ...
    $stmt = $conn->prepare("DELETE FROM vendor_service WHERE vendor_id=? AND service_id=?");
    $stmt->bind_param("ii", $vendor_id, $service_id);
    $stmt->execute();
    $stmt->close();

    // Optionally delete service if no vendor links exist
    $checkStmt = $conn->prepare("SELECT COUNT(*) as cnt FROM vendor_service WHERE service_id=?");
